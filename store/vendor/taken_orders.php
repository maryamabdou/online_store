        ...
        <div class="album py-5 bg-light">
            <div class="container">
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                    <?php
                    $display = new Users();
                    $order = $display->get_vendor_orders();
                    foreach ($order as $id) {
                        $total = 0; ?>
                        <div class="col" width="300">
                            <div class="card shadow-sm">
                                <div class="card-body">
                                    <?php $details = $display->get_vendor_orderDetails($id['id']);
                                    foreach ($details as $row) { ?>
                                        <h5 class="card-text">User:
                                            <?= $row['userName'] ?>
                                        </h5>
                                        <h5 class="card-text">Product:
                                            <?= $row['productName'] ?>
                                        </h5>
                                    ...
                                    <form method="POST" action="deliver_order.php?order_id=<?=$id['id']?>">
                                    <?php
                                    if ($id['status'] == "ordered") {
                                        echo "<button class=\"btn btn-primary my-2 my-sm-0\" type=\"submit\" name=\"enable_product\">Deliver</button>";
                                    }
                                    ?>
                                    </form>
                                </div>
